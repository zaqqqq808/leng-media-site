...
function drawSparkline(ctx: CanvasRenderingContext2D, x: number, y: number, data: number[], alpha: number, color: string) {
  const w = 36, h = 10
...
function drawBars(ctx: CanvasRenderingContext2D, x: number, y: number, data: number[], alpha: number, color: string) {
...
export default function BlobCanvas() {
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    // Skip heavy canvas animation on mobile — CSS blob fallback handles the visual
    ...
    const frame = () => {
      ...
      { const gs = ctx.createRadialGradient(cx, cy + R * 1.12, 0, cx, cy + R * 1.12, R * 0.62); gs.addColorStop(0,'rgba(0,0,0,0.22)'); gs.addColorStop(0.5,'rgba(0,0,0,0.10)'); gs.addColorStop(1,'rgba(0,0,0,0)'); ctx.fillStyle = gs; ctx.beginPath(); ctx.ellipse(cx, cy + R * 1.14, R * 0.52, R * 0.10, 0, 0, Math.PI * 2); ctx.fill() }
      for (let p = 3; p >= 1; p--) { smooth(ctx, blobPts(cx, cy, R * (1.05 + p * 0.12), t * 0.6)); ctx.fillStyle = `rgba(212,255,0,${0.035 - p * 0.004})`; ctx.fill() }
      ...
      g.addColorStop(0,'rgba(255,255,215,0.52)'); g.addColorStop(0.10,'rgba(225,255,0,0.44)'); g.addColorStop(0.32,'rgba(175,225,0,0.26)'); g.addColorStop(0.58,'rgba(100,165,0,0.15)'); g.addColorStop(0.80,'rgba(45,85,0,0.08)'); g.addColorStop(1,'rgba(0,10,0,0.02)')
      ctx.fillStyle = g; ctx.fill()
      smooth(ctx, blobPts(cx, cy, R, t))
      const shX = cx + R * 0.40, shY = cy + R * 0.44
      const sh = ctx.createRadialGradient(shX, shY, 0, cx + R * 0.14, cy + R * 0.14, R * 1.18)
      sh.addColorStop(0,'rgba(0,0,0,0.38)'); sh.addColorStop(0.35,'rgba(0,0,0,0.24)'); sh.addColorStop(0.65,'rgba(0,0,0,0.10)'); sh.addColorStop(1,'rgba(0,0,0,0)')
      ctx.fillStyle = sh; ctx.fill()
      smooth(ctx, blobPts(cx, cy, R * 0.97, t)); ctx.save(); ctx.globalAlpha = 0.06; ctx.strokeStyle = 'rgba(0,20,0,1)'; ctx.lineWidth = R * 0.18; ctx.stroke(); ctx.restore()
      smooth(ctx, blobPts(cx, cy, R * 0.99, t + 0.15)); ctx.save(); ctx.shadowColor = '#D4FF00'; ctx.shadowBlur = 30; ctx.strokeStyle = 'rgba(212,255,0,0.62)'; ctx.lineWidth = 1.5; ctx.stroke(); ctx.restore()
      smooth(ctx, blobPts(cx, cy, R * 1.005, t + 0.08)); ctx.save(); ctx.strokeStyle = 'rgba(255,255,200,0.22)'; ctx.lineWidth = 2.5; ctx.stroke(); ctx.restore()
      smooth(ctx, blobPts(cx - R * 0.12, cy - R * 0.16, R * 0.38, t * 1.12))
      const spec1 = ctx.createRadialGradient(cx - R * 0.30, cy - R * 0.32, 0, cx - R * 0.16, cy - R * 0.18, R * 0.42)
      spec1.addColorStop(0,'rgba(255,255,255,0.46)'); spec1.addColorStop(0.30,'rgba(255,255,230,0.20)'); spec1.addColorStop(0.65,'rgba(255,255,210,0.07)'); spec1.addColorStop(1,'rgba(255,255,255,0)')
      ctx.fillStyle = spec1; ctx.fill()
      const ssOx = Math.sin(t * 0.65) * R * 0.018, ssOy = Math.cos(t * 0.82) * R * 0.014
      const ssx = cx - R * 0.28 + ssOx, ssy = cy - R * 0.31 + ssOy
      ctx.beginPath(); ctx.arc(ssx, ssy, R * 0.042, 0, Math.PI * 2)
      const spec2 = ctx.createRadialGradient(ssx, ssy, 0, ssx, ssy, R * 0.042)
      spec2.addColorStop(0,'rgba(255,255,255,0.92)'); spec2.addColorStop(0.45,'rgba(255,255,240,0.28)'); spec2.addColorStop(1,'rgba(255,255,255,0)')
      ctx.fillStyle = spec2; ctx.fill()
      const ms2x = cx - R * 0.20 + ssOx * 0.5, ms2y = cy - R * 0.38 + ssOy * 0.5
      ctx.beginPath(); ctx.arc(ms2x, ms2y, R * 0.018, 0, Math.PI * 2)
      const spec3 = ctx.createRadialGradient(ms2x, ms2y, 0, ms2x, ms2y, R * 0.018)
      spec3.addColorStop(0,'rgba(255,255,255,0.55)'); spec3.addColorStop(1,'rgba(255,255,255,0)')
      ctx.fillStyle = spec3; ctx.fill()
      ;[[-0.38,-0.32,3,0.6+.35*Math.sin(t*2.1)],[0.25,-0.18,2,0.4+.25*Math.sin(t*2.6+1)],[-0.15,0.28,2.2,.35+.2*Math.sin(t*1.8+2)]].forEach(([dx,dy,r,a]) => { ctx.beginPath(); ctx.arc(cx+R*dx, cy+R*dy, r, 0, Math.PI*2); ctx.fillStyle = `rgba(255,255,200,${a})`; ctx.fill() })
      particles.forEach((p, i) => {
        p.life++; p.x += p.vx; p.y += p.vy
        const { maxLife, life, phaseOffset, item } = p
        const fadeIn = Math.min(life / 50, 1), fadeOut = Math.min((maxLife - life) / 50, 1)
        const pulse = item.bright ? 0.55 + Math.sin(t * 1.4 + phaseOffset) * 0.18 : 0.28 + Math.sin(t * 0.9 + phaseOffset) * 0.08
        p.alpha = Math.min(fadeIn, fadeOut) * pulse
        if (life >= maxLife) { particles[i] = spawnParticle(W, H); return }
        ctx.save(); ctx.globalAlpha = p.alpha; ctx.font = '10px "Space Mono", monospace'; ctx.fillStyle = item.color; ctx.fillText(item.text, p.x, p.y); ctx.restore()
